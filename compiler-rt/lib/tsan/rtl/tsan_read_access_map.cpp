...
bool ReadAccessMap::Insert(uptr addr, Sid sid) {
  ...
  const u8 swapIndex = atomic_load_acquire(&gcTracker_[index]) & 2U;
...
Pair* ReadAccessMap::FindMatchedPair(int index, uptr addr, u8 swapIndex) {
  for (int i = 0; i < kShadowCnt; i++) {
    if (static_cast<uptr>(atomic_load_acquire(&readAccessMap_[index][swapIndex][i].key)) == addr && atomic_load_acquire(&readAccessMap_[index][swapIndex][i].val) != DELETE_STATE) {
...
void ReadAccessMap::Remove(uptr addr) {
  ...
  const u8 swapIndex = gcTracker & 2U;
  ...
  for (unsigned i = 0; i < kShadowCnt; i++) {
    ...
    if(gcInProgress || !atomic_compare_exchange_strong(&gcTracker_[index],&gcTracker,gcTracker|1U,memory_order_acquire))
      return;  
    ...
    for(unsigned j=0;j<kShadowCnt;j++){
      ...
      if(!atomic_compare_exchange_strong(&swapPair[j].val,&oldVal,curVal,memory_order_relaxed)){
        atomic_store_release(&gcTracker_[index],gcTracker);
        return;
      }

      //memory_order_release is required to ensure the previous CAS won't get reordered after this CAS.
      if(!atomic_compare_exchange_strong(&curPair[j].val,&curVal,DELETE_STATE,memory_order_release)){
        atomic_store_release(&gcTracker_[index],gcTracker);
        return;
      }
    }
...
bool ReadAccessMap::Contain(uptr addr, Sid sid){
  ...
  if(Pair* pair = FindMatchedPair(index,addr,swapIndex)){
    ...
    u64 sidSlot = static_cast<u64>(sid);
    u64 curSlot = (1ULL<<9)-1ULL;
    for(unsigned j=0;j<sizeof(u64)/sizeof(Sid);j++){
      if(cell & curSlot == sidSlot)
        return true;
      sidSlot<<=sizeof(Sid)*8;
      curSlot<<=sizeof(Sid)*8;
...
u64 ReadAccessMap::Get(uptr addr){
  ...
  for(unsigned i=0;i<kShadowCnt;i++){
    ...
    if(atomic_load_acquire(&readAccessMap_[index][!swapIndex][i].key) == addr){
...
bool ReadAccessMap::UpdateCell(Pair* pair, u64 cell, Sid sid){
  ...
  for(unsigned j=0;j<sizeof(u64)/sizeof(Sid);j++){
    ...
    if(cell & curSlot == curSlot){
      ...
      curSlot |= sidSlot;
      return atomic_compare_exchange_strong(&pair->val,&cell,curSlot,memory_order_release);
    }
    curSlot<<=sizeof(Sid)*8;
    sidSlot<<=sizeof(Sid)*8;
